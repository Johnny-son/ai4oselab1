...
// 定义内核入口点：分配内核栈
// 增大栈空间以容纳 Process 结构体（含 syscall_counts: [usize; 512] = 4KB）
// 在栈上的临时构造
#[cfg(target_arch = "riscv64")]
...
extern "C" fn rust_main() -> ! {
    ...
    // 增大到 8 页以容纳系统调用处理中的栈使用
    const SCHED_PAGES: usize = 8;
    const PAGE: Layout =
        unsafe { Layout::from_size_align_unchecked(SCHED_PAGES << Sv39::PAGE_BITS, 1 << Sv39::PAGE_BITS) };
    let pages = SCHED_PAGES;
    ...
    while !unsafe { PROCESSES.get_mut().is_empty() } {
        ...
        match scause::read().cause() {
            ...
            scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                ...
                unsafe { PROCESSES.get_mut()[0].record_syscall(id) };
                ...
                match tg_syscall::handle(Caller { entity: 0, flow: 0 }, id, args) {
                    Ret::Done(ret) => match id {
                        ...
                        _ => {
                            let processes = unsafe { PROCESSES.get_mut() };
                            let ctx = &mut processes[0].context.context;
