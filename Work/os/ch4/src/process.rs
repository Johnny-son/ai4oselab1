...
/// 可记录的系统调用编号上限。
///
/// Linux 兼容 ABI 下常见系统调用号小于 511，本章实验只需支持 trace/clock 等有限调用。
pub const SYSCALL_COUNT_MAX: usize = 512;
...
pub struct Process {
    ...
    /// 系统调用计数表：按系统调用号统计本进程的调用次数
    pub syscall_counts: [usize; SYSCALL_COUNT_MAX],
}
...
impl Process {
    ...
    /// 记录一次系统调用。
    #[inline]
    pub fn record_syscall(&mut self, id: SyscallId) {
        let idx = id.0;
        if idx < SYSCALL_COUNT_MAX {
    ...
    pub fn syscall_count(&self, id: usize) -> Option<usize> {
        (id < SYSCALL_COUNT_MAX).then_some(self.syscall_counts[id])
    }
}
