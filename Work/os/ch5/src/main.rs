...
mod impls {
    use crate::{
        APPS, PROCESSOR, Sv39, build_flags, parse_flags,
        process::{MIN_PRIORITY, Process as ProcStruct},
    ...
    impl PageManager<Sv39> for Sv39Manager {
        ...
        fn deallocate(&mut self, pte: Pte<Sv39>, len: usize) -> usize {
            ...
            let ptr = self.p_to_v::<u8>(pte.ppn());
            unsafe {
                dealloc(
                    ptr.as_ptr(),
    ...
    impl Process for SyscallContext {
        ...
        fn spawn(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            const READABLE: VmFlags<Sv39> = build_flags("RV");
            let processor_ptr: *mut PManager<ProcStruct, ProcManager> =
                PROCESSOR.get_mut() as *mut _;
            let mut path_buf = Vec::with_capacity(count);
            let (parent_pid, parent_priority, parent_stride) = unsafe {
                let processor = &mut *processor_ptr;
                ...
                let slice = core::slice::from_raw_parts(ptr.as_ptr(), count);
                path_buf.extend_from_slice(slice);
                (current.pid, current.priority, current.stride)
            };

            let trimmed_len = path_buf
                .iter()
                .position(|&b| b == 0)
                .unwrap_or(path_buf.len());
            let app_name = match core::str::from_utf8(&path_buf[..trimmed_len]) {
                Ok(name) if !name.is_empty() => name,
                _ => return -1,
            };

            let Some(&app_data) = APPS.get(app_name) else {
                return -1;
            };
            let elf = match ElfFile::new(app_data) {
                Ok(elf) => elf,
                Err(_) => return -1,
            };
            let mut child = match ProcStruct::from_elf(elf) {
                Some(proc) => proc,
                None => return -1,
            };
            child.priority = parent_priority;
            child.stride = parent_stride;
    ...
    impl Memory for SyscallContext {
        ...
        fn mmap(
            ...
            let end_vpn = VAddr::new(end).ceil();
