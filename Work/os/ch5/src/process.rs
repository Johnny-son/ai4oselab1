...
pub struct Process {
    ...
    /// 当前进程的调度优先级（越小表示优先级越低）
    pub priority: usize,
    /// stride 调度算法使用的累计步长
    pub stride: i64,
}

/// stride 调度的全局步长，保证整数除法时的精度
pub const BIG_STRIDE: i64 = 1 << 20;
/// 默认进程优先级
...
impl Process {
    ...
    /// 计算当前优先级对应的 stride 增量
    #[inline]
    pub fn stride_step(&self) -> i64 {
        let prio = max(self.priority, MIN_PRIORITY);
        max(1, BIG_STRIDE / prio as i64)
    }
}
